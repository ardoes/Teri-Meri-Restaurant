"use client";

import { useRef } from "react";
import Link from "next/link";
import { gsap, useGSAP } from "@/lib/gsap";
import { Wordmark } from "@/components/ui/Wordmark";
import {
  FOOTER_REVEAL_VH,
  getLenisScrub,
} from "@/lib/scroll-constants";
import {
  RESTAURANT_INSTAGRAM_URL,
  RESTAURANT_MAPS_URL,
} from "@/lib/site-seo";

const HOURS = [{ d: "Daily", t: "12:30 PM – 12:30 AM" }];

export function SiteFooter() {
  const root = useRef<HTMLElement>(null);

  useGSAP(
    () => {
      const reduced = window.matchMedia(
        "(prefers-reduced-motion: reduce)"
      ).matches;
      if (reduced) return;

      const footer = root.current;
      if (!footer) return;

      const panels = footer.querySelectorAll<HTMLElement>(".f-panel");
      const leftPanel = panels[0];
      const rightPanel = panels[1];
      if (!leftPanel) return;

      const mark = leftPanel.querySelector<HTMLElement>(".f-mark");
      const leftItems = leftPanel.querySelectorAll<HTMLElement>(".f-item");
      const rightItems =
        rightPanel?.querySelectorAll<HTMLElement>(".f-item") ?? [];

      gsap.set(panels, { clipPath: "inset(100% 0% 0% 0%)" });
      if (mark) gsap.set(mark, { y: 28, opacity: 0 });
      gsap.set(leftItems, { y: 18, opacity: 0 });
      gsap.set(rightItems, { y: 18, opacity: 0 });

      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: footer,
          start: "top bottom",
          end: () => `+=${window.innerHeight * FOOTER_REVEAL_VH}`,
          scrub: getLenisScrub(),
        },
      });

      tl.fromTo(
        leftPanel,
        { clipPath: "inset(100% 0% 0% 0%)" },
        { clipPath: "inset(0% 0% 0% 0%)", duration: 0.45, ease: "none" },
        0
      );

      if (rightPanel) {
        tl.fromTo(
          rightPanel,
          { clipPath: "inset(100% 0% 0% 0%)" },
          { clipPath: "inset(0% 0% 0% 0%)", duration: 0.45, ease: "none" },
          0.08
        );
      }

      if (mark) {
        tl.fromTo(
          mark,
          { y: 28, opacity: 0 },
          { y: 0, opacity: 1, duration: 0.35, ease: "none" },
          0.1
        );
      }

      tl.fromTo(
        leftItems,
        { y: 18, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.28, stagger: 0.04, ease: "none" },
        0.12
      );

      if (rightItems.length) {
        tl.fromTo(
          rightItems,
          { y: 18, opacity: 0 },
          { y: 0, opacity: 1, duration: 0.28, stagger: 0.04, ease: "none" },
          0.18
        );
      }
    },
    { scope: root }
  );

  return (
    <footer
      ref={root}
      className="site-footer grid w-full grid-cols-1 md:grid-cols-[7fr_3fr]"
    >
      {/* LEFT — dark muted orange, ~70% */}
      <div className="site-footer-body f-panel flex min-h-[60vh] flex-col justify-between overflow-hidden bg-orange-muted py-16 md:py-24">
        <div className="site-footer-pad flex h-full min-h-[inherit] flex-col justify-between">
          <p className="f-item eyebrow text-cream/60">Teri Meri · Yours &amp; Mine</p>

          <div className="f-mark">
            <Wordmark
              className="text-[clamp(3.5rem,12vw,11rem)]"
              teriClassName="text-cream"
              meriClassName="text-forest"
            />
            <p className="mt-6 max-w-md text-lg text-cream/75">
              Heritage recipes, fresh spice and slow fire — a table set for two,
              and everyone they love.
            </p>
          </div>

          <p className="f-item text-sm text-cream/60">
            © 2025 Teri Meri. Crafted with fire, spice &amp; soul.
          </p>
        </div>
      </div>

      {/* RIGHT — deep forest green, ~30% */}
      <div className="f-panel flex min-h-[60vh] flex-col justify-between overflow-hidden bg-forest py-16 text-cream md:py-24">
        <div className="site-footer-pad site-footer-pad--narrow flex h-full min-h-[inherit] flex-col justify-between">
          <div className="f-item">
            <p className="eyebrow text-gold-soft">Visit</p>
            <address className="mt-4 not-italic">
              <Link
                href={RESTAURANT_MAPS_URL}
                target="_blank"
                rel="noopener noreferrer"
                data-cursor="hover"
                className="text-cream/85 underline decoration-transparent underline-offset-4 transition-colors hover:text-cream hover:decoration-gold/50"
              >
                Teri Meri Biryani &amp; Restaurant
                <br />
                Al Muallifin Street, Al Aziziyah District
                <br />
                Jeddah 23342, Saudi Arabia
                <br />
                JCZB2832
              </Link>
            </address>
            <Link
              href="tel:+966535004311"
              data-cursor="hover"
              className="mt-4 inline-block text-cream/70 underline decoration-gold/50 underline-offset-4 transition-colors hover:text-cream"
            >
              +966 53 500 4311
            </Link>
          </div>

          <div className="f-item">
            <p className="eyebrow text-gold-soft">Hours</p>
            <ul className="mt-4 space-y-2 text-cream/85">
              {HOURS.map((h) => (
                <li
                  key={h.d}
                  className="flex flex-col gap-1 text-sm md:flex-row md:justify-between md:gap-6"
                >
                  <span>{h.d}</span>
                  <span className="text-cream/55 md:text-right">{h.t}</span>
                </li>
              ))}
            </ul>
          </div>

          <div className="f-item">
            <p className="eyebrow text-gold-soft">Follow</p>
            <ul className="mt-4 space-y-2">
              <li>
                <Link
                  href={RESTAURANT_INSTAGRAM_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  data-cursor="hover"
                  className="text-cream/75 transition-colors hover:text-gold-soft"
                >
                  Instagram
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </footer>
  );
}
